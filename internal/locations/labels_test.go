...
func TestGenerateLabels_Single(t *testing.T) {
	got, err := GenerateLabels("single", LabelParams{Label: "junk-box"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateLabels_Row(t *testing.T) {
	got, err := GenerateLabels("row", LabelParams{Prefix: "box", From: 1, To: 5})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateLabels_Grid(t *testing.T) {
	got, err := GenerateLabels("grid", LabelParams{Prefix: "shelf", RowFrom: "A", RowTo: "B", ColFrom: 1, ColTo: 2})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateLabels_Errors(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if _, err := GenerateLabels(tc.method, tc.p); err == nil {
...
func TestGenerateLabels_RowSingleValue(t *testing.T) {
	...
	got, err := GenerateLabels("row", LabelParams{Prefix: "x", From: 3, To: 3})
	if err != nil {
		t.Fatal(err)
	}
	want := []string{"x3"}
