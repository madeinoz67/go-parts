...
go-parts is one Go binary over an embedded **Pebble** KV store, with five
interface layers (RPC, REST, MCP, Website, TUI) over a single core service.
...
The authoritative design is the [PRD](../internals/go-parts-prd.md); section
references above point into it.
